...
import { TrazabilidadProductosService } from 'src/app/services/premios-tangibles/trazabilidad-productos/trazabilidad-productos.service';

@Component({
  selector: 'app-trazabilidad-productos',
  templateUrl: './trazabilidad-productos.component.html',
  styleUrls: ['./trazabilidad-productos.component.scss']
})
export class TrazabilidadProductosComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private router: Router, private spinner: NgxSpinnerService, private TrazabilidadProductosService: TrazabilidadProductosService, private route: ActivatedRoute) {
  ...
  listar() {
    this.TrazabilidadProductosService.listar().subscribe(
      (data) => {
        if (data) {
          if (data.codigo == 200) {
            this.data = [...data.data];
  ...
  guardar() {
    ...
    setTimeout(() => {
      this.TrazabilidadProductosService.crear(formValues).subscribe(
        (data) => {
          this.spinner.hide();
          if (data) {
