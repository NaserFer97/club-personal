...
import { BajaPremioService } from 'src/app/services/premios-tangibles/baja-premio/baja-premio.service';

@Component({
  selector: 'app-baja-premio',
  templateUrl: './baja-premio.component.html',
  styleUrls: ['./baja-premio.component.scss']
})
export class BajaPremioComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private router: Router, private spinner: NgxSpinnerService, private BajaPremioService: BajaPremioService, private route: ActivatedRoute) {
  ...
  listar() {
    this.BajaPremioService.listar().subscribe(
      (data) => {
        if (data) {
          if (data.codigo == 200) {
            this.data = [...data.data];
  ...
  guardar() {
    ...
    setTimeout(() => {
      this.BajaPremioService.crear(formValues).subscribe(
        (data) => {
          this.spinner.hide();
          if (data) {
